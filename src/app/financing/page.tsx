import { Metadata } from "next";
import SectionTitle from "@/components/global/sectionTitle";
import Form from "@/components/financing/form";

export const metadata: Metadata = {
  title: "Used Cars Dealership in Waterloo - Tricity Auto",
  description:
    "Tricity Auto provides flexible car loans in Waterloo with guaranteed approvals and a 0% turn-down policy. Get fast approval and the best rates tailored to your budget.",
  openGraph: {
    title: "Used Cars Dealership in Waterloo - Tricity Auto",
    description:
      "Tricity Auto provides flexible car loans in Waterloo with guaranteed approvals and a 0% turn-down policy. Get fast approval and the best rates tailored to your budget.",
    url: "https://tricityautofinance.com/financing",
    siteName: "Tricity Auto - Waterloo, ON Used Car Dealership",
    locale: "en_US",
    type: "website",
  },
};

const Financing = () => {
  return (
    <section className="py-desktop px-mobile md:px-tablet lg:px-desktop">
      {/* HEADER */}
      <div className="flex flex-col items-center justify-between lg:flex-row mb-50 gap-50">
        <SectionTitle title={"title"} containerClassName={"!mb-0 title"} isH1 />
        <p className="mt-10 text-grayMouse lg:mt-0 title">description</p>
      </div>

      <Form />
    </section>
  );
};

export default Financing;
